...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch the last 60 minutes of data for the given symbol
    ...
    const { data, error } = await supabaseAdmin
      ...
      .gt('recorded_at', oneHourAgo)
      .order('recorded_at', { ascending: true });

    if (error) throw error;

    // Return just an array of the price numbers
    const priceData = data.map(item => item.price);

    return new Response(JSON.stringify({ history: priceData }), {
