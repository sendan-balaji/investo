...
export const LoginPage: React.FC<LoginPageProps> = ({ onEmailLogin, onGoogleLogin }) => {
    ...
    return (
        <div className="bg-surface min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            ...
            <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        <div>
                            ...
                            <div className="mt-2">
                                <input id="email" name="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                                    className="block w-full rounded-md border-0 py-2.5 px-4 bg-surface text-text-primary shadow-sm ring-1 ring-inset ring-border placeholder:text-text-secondary focus:ring-2 focus:ring-inset focus:ring-accent"
                                />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-2 relative">
                                <input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)}
                                    className="block w-full rounded-md border-0 py-2.5 pl-4 pr-10 bg-surface text-text-primary shadow-sm ring-1 ring-inset ring-border placeholder:text-text-secondary focus:ring-2 focus:ring-inset focus:ring-accent"
                                />
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-sm leading-6">
                                <a href="#" className="font-semibold text-accent hover:opacity-80">
                                    Forgot password?
                                </a>
                            </div>
                        </div>
                    ...
                    <div className="mt-10">
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center text-sm font-medium leading-6">
                                <span className="bg-background px-6 text-text-primary">Or continue with</span>
                            </div>
                        </div>
                ...
                <p className="mt-10 text-center text-sm text-text-secondary">
                    Not a member?{' '}
                    <Link to="/signup" className="font-semibold leading-6 text-accent hover:opacity-80">
                        Sign up for free
                    </Link>
                </p>
            </div>
