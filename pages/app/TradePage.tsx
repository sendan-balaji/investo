...
const OrderPreviewModal: React.FC<{ order: any; onConfirm: () => void; onCancel: () => void; }> = ({ order, onConfirm, onCancel }) => (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 animate-fade-in">
        <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm m-4">
            <h2 className="text-xl font-bold text-center mb-1 text-gray-900">Confirm Order</h2>
            <p className="text-sm text-center text-gray-500 mb-6">Please review your order details below.</p>
            <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center">
                    <span className="text-gray-500">Action</span>
                    <span className={`font-bold text-lg ${order.side === 'BUY' ? 'text-green-600' : 'text-red-600'}`}>{order.side} {order.symbol}</span>
                </div>
                <div className="flex justify-between items-center">
                    <span className="text-gray-500">Quantity</span>
                    <span className="font-semibold text-gray-800">{order.quantity} Shares</span>
                </div>
                <div className="flex justify-between items-center">
                 ...
                 <div className="flex justify-between items-center pt-3 mt-3 border-t">
                    ...
                    <span className="font-bold text-lg text-gray-900">₹{(order.quantity * order.price).toLocaleString('en-IN')}</span>
                </div>
            </div>
            <div className="mt-8 grid grid-cols-2 gap-4">
                <button onClick={onCancel} className="py-2.5 px-4 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 transition-colors">Cancel ❌</button>
...
const SuccessToast: React.FC<{ message: string; onDismiss: () => void; isVisible: boolean }> = ({ message, onDismiss, isVisible }) => (
     <div className={`fixed top-24 right-6 bg-white border border-green-300 shadow-lg rounded-lg p-4 flex items-center gap-3 z-50 transition-all duration-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
...
const WatchlistPanel: React.FC<{ onSelectStock: (symbol: string) => void, selectedStock: any }> = ({ onSelectStock, selectedStock }) => {
    return (
        <div className="bg-white h-full border border-gray-200 rounded-xl shadow-sm flex flex-col">
            <div className="p-4 border-b border-gray-200">
                <div className="relative">
                 ...
                 <div className="mt-4">
                    <label htmlFor="category" className="text-xs font-semibold text-gray-500 uppercase">Categories</label>
                    <select id="category" className="w-full mt-1 px-2 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
            ...
            <div className="flex-1 overflow-y-auto">
                {mockWatchlist.map(stock => (
                    <button key={stock.symbol} onClick={() => onSelectStock(stock.symbol)} className={`w-full text-left px-4 py-3 flex justify-between items-center transition-colors ${selectedStock.symbol === stock.symbol ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
                        <div>
                            <p className={`font-bold ${selectedStock.symbol === stock.symbol ? 'text-indigo-600' : 'text-gray-800'}`}>{stock.symbol}</p>
                            <p className="text-xs text-gray-500">{stock.name}</p>
                        </div>
                        <div className="text-right">
                            <p className="font-semibold text-gray-900">₹{stock.price.toFixed(2)}</p>
                            <p className={`text-xs font-medium ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>{stock.change.toFixed(2)}%</p>
                        </div>
                    </button>
                ))}
            </div>
...
const OrderPlacementPanel: React.FC<{ selectedStock: any; onPlaceOrder: (order: any) => void, orderSide: 'BUY' | 'SELL', setOrderSide: (side: 'BUY' | 'SELL') => void, quantityInputRef: React.RefObject<HTMLInputElement> }> = ({ selectedStock, onPlaceOrder, orderSide, setOrderSide, quantityInputRef }) => {
    ...
    return (
        <div className="bg-white h-full border border-gray-200 rounded-xl shadow-sm p-4 flex flex-col">
            <div className="grid grid-cols-2 gap-2 mb-4">
                <button onClick={() => setOrderSide('BUY')} className={`py-2 text-lg font-bold rounded-lg transition-all ${orderSide === 'BUY' ? 'bg-green-600 text-white shadow-md' : 'bg-gray-100 text-green-700 hover:bg-green-50'}`}>BUY</button>
                <button onClick={() => setOrderSide('SELL')} className={`py-2 text-lg font-bold rounded-lg transition-all ${orderSide === 'SELL' ? 'bg-red-600 text-white shadow-md' : 'bg-gray-100 text-red-700 hover:bg-red-50'}`}>SELL</button>
            </div>
            <div className="space-y-4">
                <div>
                    <label className="text-sm font-medium text-gray-500">Quantity</label>
                    <div className="relative flex items-center mt-1">
                        <button onClick={() => setQuantity(q => Math.max(1, q-1))} className="absolute left-0 px-3 py-2 text-gray-600 bg-gray-100 rounded-l-lg hover:bg-gray-200">-</button>
                        <input ref={quantityInputRef} type="number" value={quantity} onChange={e => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-full text-center px-10 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        <button onClick={() => setQuantity(q => q+1)} className="absolute right-0 px-3 py-2 text-gray-600 bg-gray-100 rounded-r-lg hover:bg-gray-200">+</button>
                    </div>
                </div>
                <div>
                    <label className="text-sm font-medium text-gray-500">Order Type</label>
                    <select value={orderType} onChange={e => setOrderType(e.target.value)} className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 text-sm space-y-2">
                <div className="flex justify-between"><span>Required Funds:</span> <span className="font-semibold">₹{requiredFunds}</span></div>
                 <div className="flex justify-between"><span>Available Funds:</span> <span className="font-semibold">₹2,45,000.00</span></div>
            </div>
    ...
    <div className="flex items-center justify-between pb-4">
        <div className="flex items-center gap-4">
            <div>
             ...
             <span className="px-2.5 py-1 text-xs font-semibold text-green-800 bg-green-100 rounded-full">MARKET OPEN</span>
        </div>
        ...
        <div className="text-right">
            ...
            <div className={`flex items-center justify-end gap-1 text-lg font-semibold ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
...
const MarketDepth: React.FC<{ data: typeof mockMarketDepth }> = ({ data }) => {
    ...
    const DepthRow: React.FC<{ orders: number, qty: number, price: number, type: 'buy' | 'sell' }> = ({ orders, qty, price, type }) => {
        ...
        return (
            <div className="grid grid-cols-3 gap-2 text-xs relative text-center py-0.5">
                <div className={`absolute top-0 h-full ${type === 'buy' ? 'right-0 bg-green-100' : 'left-0 bg-red-100'}`} style={{ width: `${percentage}%` }}></div>
                <span className="z-10">{type === 'buy' ? orders : price.toFixed(2)}</span>
                <span className="z-10 font-semibold">{qty}</span>
                <span className={`z-10 font-semibold ${type === 'buy' ? 'text-green-600' : 'text-red-600'}`}>{type === 'buy' ? price.toFixed(2) : orders}</span>
            </div>
        );
    };

    return (
        <div className="h-full border-l border-gray-200 pl-4 text-xs">
            <h3 className="font-bold text-center text-sm mb-2 text-gray-700">Market Depth</h3>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <div className="grid grid-cols-3 gap-2 text-gray-500 font-semibold text-center pb-1 border-b">
            ...
            <div className="flex justify-between mt-2 pt-2 border-t text-sm">
                <div className="text-center"><p className="font-bold text-green-600">{totalBuyQty}</p><p className="text-gray-500 text-xs">Total Bids</p></div>
                <div className="text-center"><p className="font-bold text-red-600">{totalSellQty}</p><p className="text-gray-500 text-xs">Total Asks</p></div>
            </div>
        </div>
    );
};
...
const CandlestickChart: React.FC = () => (
    <svg width="100%" height="100%" viewBox="0 0 500 250" preserveAspectRatio="none">
        <line x1="30" y1="230" x2="480" y2="230" stroke="#d1d5db" strokeWidth="1" />
        <line x1="30" y1="20" x2="30" y2="230" stroke="#d1d5db" strokeWidth="1" />
        <text x="25" y="25" textAnchor="end" fontSize="10" fill="#6b7280">1400</text>
        <text x="25" y="125" textAnchor="end" fontSize="10" fill="#6b7280">1380</text>
        <text x="25" y="225" textAnchor="end" fontSize="10" fill="#6b7280">1360</text>
        ...
        switch (activeTab) {
            case 'Positions':
                return (<table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50"><tr><th scope="col" className="px-4 py-2">Symbol</th><th scope="col" className="px-4 py-2">Qty</th><th scope="col" className="px-4 py-2">Avg. Price</th><th scope="col" className="px-4 py-2">LTP</th><th scope="col" className="px-4 py-2">P&L</th></tr></thead>
                        <tbody>{mockPositions.map(p => (<tr key={p.symbol} className="bg-white border-b"><td className="px-4 py-2 font-medium text-gray-900">{p.symbol}</td><td className="px-4 py-2">{p.qty}</td><td className="px-4 py-2">₹{p.avgPrice.toFixed(2)}</td><td className="px-4 py-2">₹{p.ltp.toFixed(2)}</td><td className={`px-4 py-2 font-semibold ${p.pnl >=0 ? 'text-green-600' : 'text-red-600'}`}>{p.pnl >= 0 ? '+' : ''}₹{p.pnl.toFixed(2)}</td></tr>))}</tbody></table>);
            case 'Orders':
                 return (<table className="w-full text-sm text-left">
                        ...
                        <tbody>{mockOrders.map((o, i) => (<tr key={i} className="bg-white border-b"><td className="px-4 py-2 font-medium text-gray-900">{o.symbol}</td><td className="px-4 py-2">{o.type}</td><td className={`px-4 py-2 font-semibold ${o.side === 'BUY' ? 'text-green-600' : 'text-red-600'}`}>{o.side}</td><td className="px-4 py-2">{o.qty}</td><td className="px-4 py-2">₹{o.price.toFixed(2)}</td><td className="px-4 py-2"><span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${o.status === 'OPEN' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>{o.status}</span></td></tr>))}</tbody></table>);
            case 'Trade History':
                return (<table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50"><tr><th scope="col" className="px-4 py-2">Symbol</th><th scope="col" className="px-4 py-2">Side</th><th scope="col" className="px-4 py-2">Qty</th><th scope="col" className="px-4 py-2">Price</th><th scope="col" className="px-4 py-2">Time</th></tr></thead>
                        <tbody>{mockTradeHistory.map((t, i) => (<tr key={i} className="bg-white border-b"><td className="px-4 py-2 font-medium text-gray-900">{t.symbol}</td><td className={`px-4 py-2 font-semibold ${t.side === 'BUY' ? 'text-green-600' : 'text-red-600'}`}>{t.side}</td><td className="px-4 py-2">{t.qty}</td><td className="px-4 py-2">₹{t.price.toFixed(2)}</td><td className="px-4 py-2">{t.time}</td></tr>))}</tbody></table>);
            default: return null;
        }
    };
    
    return (
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col h-full"><div className="flex border-b border-gray-200">{['Positions', 'Orders', 'Trade History'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-semibold transition-colors ${activeTab === tab ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>{tab}</button>))}</div><div className="flex-1 overflow-y-auto">{renderContent()}</div></div>
    );
};
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-11rem)]">
            ...
            <div className="lg:col-span-3 h-full flex flex-col gap-6">
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 flex-grow flex flex-col">
